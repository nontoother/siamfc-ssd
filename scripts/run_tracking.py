...
def main(checkpoint, input_files):
    ...
    with tf.Session(graph=g, config=sess_config) as sess:
        ...
        if TRACK_MODE == 1:
            ...
            tracker = Tracker2(model, model_config=model_config, track_config=track_config)
        ...
        for video_dir in video_dirs:
            ...
            filenames = sort_nicely(glob(video_dir + '/*.jpg'))
            first_line = open(video_dir + '/groundtruth.txt').readline()
            bb = [int(v) for v in first_line.strip().split(',')]
            init_bb = Rectangle(bb[0] - 1, bb[1] - 1, bb[2], bb[3])  # 0-index in python, 第一帧的真实框
