...
def _configure_learning_rate(train_config, global_step):
    ...
    num_batches_per_epoch = \
        int(train_config['train_data_config']['num_examples_per_epoch'] / train_config['train_data_config'][
            'batch_size'])
...
def main(model_config, train_config, track_config):
    ...
    with g.as_default():
        ...
        for step in range(start_step, total_steps):
            ...
            _, loss, batch_loss, cls_loss, reg_loss, response, debug_test = sess.run([train_op, model.total_loss, model.batch_loss, model.cls_loss, model.reg_loss, model.response, model.debug_test])
            ...
            if step % 10 == 0:
                ...
                h, m = divmod(m, 60)
                format_str = ('%s: step %d, cls_loss = %.2f, reg_loss = %.2f, total loss = %.2f, batch loss = %.2f (%.1f examples/sec; %.3f '
                              'sec/batch; %dh:%02dm:%02ds remains)')
                # print(response)
                # print(debug_test)
                logging.info(format_str % (datetime.now(), step, cls_loss, reg_loss, loss, batch_loss,
