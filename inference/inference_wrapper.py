...
class InferenceWrapper():
    ...
    """
        ...
        #  推理的时候的三个尺度，现在改成了一个
        num_scales = track_config['num_scales']  # track_config['num_scales']=1
        scales = np.arange(num_scales) - get_center(num_scales)
        assert np.sum(scales) == 0, 'scales should be symmetric'
        search_factors = [track_config['scale_step'] ** x for x in scales]  # track_config['scale_step']=1

        frame_sz = tf.shape(self.image)  # 1920*1080
        target_yx = self.target_bbox_feed[0:2]  # 目标框中心点坐标
        target_size = self.target_bbox_feed[2:4]  # 目标框大小
        ...
        self.search_images = image_cropped + avg_chan  # [255, 255, 3]
    ...
    def build_detection(self):
        self.embeds = self.get_image_embedding(self.search_images, reuse=True)  # self.search_images:[3, 255, 255, 3]; self.embeds:[3, 22, 22, 256]
    ...
    def reg_convolution(self):
      with tf.variable_scope('reg_conv'):
        shape_exem = self.templates.shape.as_list()
        reg_feature = slim.conv2d(self.embeds, 4, [shape_exem[1], shape_exem[2]], padding='VALID', scope='reg')
        self.reg_pred = tf.reshape(tf.squeeze(reg_feature), (-1, 4))
    ...
    def inference_step(self, sess, input_feed):
        ...
        image_cropped, scale_xs, response_up, response, reg_pred = sess.run(
            fetches=[image_cropped_op, self.scale_xs, self.response_up, self.response, self.reg_pred],  # 关掉上采样，不用response_up改用response
            feed_dict={
