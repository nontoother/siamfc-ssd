        ...
        for i, filename in enumerate(frames):
            if i > 0 or include_first:  # We don't really want to process the first image unless intended to do so.
                ...
                if self.log_level > 0:
                    ...
                    for j in range(m):
                        ...
                        if riou >= 0.4:
                            ...
                            if reg_pred[j, 0] + reg_pred[j, 1] + reg_pred[j, 2] + reg_pred[j, 3] == 0:
                                ...
                                alpha = (riou - 0.4) / 0.15
                                image_cropped_to_write = cv2.rectangle(image_cropped_to_write, p1, p2, (alpha * 255, 0, 0))
                                ...
                                image_cropped_to_write = cv2.rectangle(image_cropped_to_write, p1, p2, (0, alpha * 255, 0), thickness=2)
                    ...
                    ####################################################################################################
                    image = cv2.imread(filename)
                    temp_x += (box[0] - get_center(response_size))/search_scale_list[best_scale]
                    temp_y += (box[1] - get_center(response_size))/search_scale_list[best_scale]
                    w = box[2] / search_scale_list[best_scale]
                    h = box[3] / search_scale_list[best_scale]
                    p1 = (int(np.round(temp_x - w / 2)), int(np.round(temp_y - h / 2)))
                    p2 = (int(np.round(temp_x + w / 2)), int(np.round(temp_y + h / 2)))
                    img = cv2.rectangle(image, p1, p2, (0, 0, 255), thickness=2)
                    imwrite(osp.join(logdir, 'test/image_{}.jpg'.format(i)), img)
