...
def get_crops(im, bbox, size_z, size_x, context_amount):
    ...
    Args:
      ...
      size_z: Target + context size, 127
      size_x: The resultant crop size, 255
    ...
    image_crop_x, _, _, _, _, new_box = get_subwindow_avg(im, [cy, cx], [w, h],
                                                 [size_x, size_x],
                                                 [np.round(s_x), np.round(s_x)])

    # image_crop_x = cv2.rectangle(image_crop_x, (new_box[0], new_box[1]), (new_box[2], new_box[3]), (0, 0, 255))
    # cv2.imshow('imgcrop', image_crop_x)
    # cv2.waitKey(0)

    return image_crop_x, new_box, scale_x


def get_subwindow_avg(im, pos, box_size, model_sz, original_sz):
    ...
    if not (model_sz[0] == original_sz[0] and model_sz[1] == original_sz[1]):
        ...
        scale_w = original_sz[0]/model_sz[0]
        scale_h = original_sz[1]/model_sz[1]
    else:
        im_patch = im_patch_original
        scale_w, scale_h = 1, 1

    w = box_size[0]/scale_w
    h = box_size[1]/scale_h
    # im_patch = cv2.rectangle(im_patch, (int(127-w/2), int(127-h/2)), (int(127+w/2), int(127+h/2)), (0, 0, 255))
    # cv2.imshow('img2', im_patch)
    # cv2.waitKey(0)
    return im_patch, left_pad, top_pad, right_pad, bottom_pad, (int(127-w/2), int(127-h/2), int(127+w/2), int(127+h/2))
