...
class RandomStretch(object):
    ...
    def __call__(self, img):
        scale = 1.0 + tf.random_uniform([], -self.max_stretch, self.max_stretch)  # [0.5, 1.5]之间的数
        img_shape = tf.shape(img)  # img_shape[0] is height, img_shape[1] is width
        ts = tf.to_int32(tf.round(tf.to_float(img_shape[:2]) * scale))  # height and width shrink at same scale
...
class CenterCrop(object):
    ...
    def __call__(self, img):
        th, tw = self.size
        return tf.image.resize_image_with_crop_or_pad(img, th, tw)  # 通过集中裁剪图像或将其均匀填充为零，将图像调整为目标宽度和高度。
...
class RandomCrop(object):
    ...
    def __call__(self, img):
        ...
        y1 = tf.random_uniform([], 0, img_shape[0] - th, dtype=tf.int32)  # 随机返回一个0-16的数字
