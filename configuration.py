...
then following the examples in the `experiments` directory.
"""
...
TRAIN_CONFIG = {
  ...
  'train_data_config': {'input_imdb': '__Data/train_imdb.pickle',
                        'preprocessing_name': 'siamese_fc_color',
                        'num_examples_per_epoch': 1000,
                        'epoch': 50,
                        'batch_size': 4,
                        'max_frame_dist': 50,  # Maximum distance between any two random frames draw from videos.
                        'prefetch_threads': 4,
                        'prefetch_capacity': 15 * 8, },  # The maximum elements number in the data loading queue

  'validation_data_config': {'input_imdb': '__Data/validation_imdb.pickle',
  ...
  'save_model_every_n_step': 1250,  # save model every epoch
...
TRACK_CONFIG = {
  ...
  # Configurations for searching scales
  'num_scales': 1,  # Number of scales to search
  'scale_step': 1,  # Scale changes between different scale search
  'scale_damp': 0.59,  # Damping factor for scale update
  'scale_penalty': 0,  # 当num_scales为1的时候，用不上
